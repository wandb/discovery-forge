...
Every `build_*_agent(output_dir, registry, day, ...)` factory captures paths and the registry **in a closure** before declaring `@function_tool` callables. The agent never receives `output_dir` as a tool argument — the model would have to guess it. Always extend agents by adding a new `@function_tool` inside the existing factory body, not by passing more state through arguments.

### Global registry + per-day change log

`daily_runs/_registry/` is the persistent source of truth. `tools.jsonl` indexes every tool ever profiled (URL-normalized for dedup); `profiles/{slug}.md` stores the canonical `ToolProfile` YAML+body. Each `daily_runs/{day}/` folder records only the deltas: `_new_candidates.jsonl` (first profiled today), `_updated_tools.jsonl` (metadata changed), and `highlights.md` (auto-derived from those two).

`ToolRegistry.add(profile, day)` returns `True` for "new" and `False` for "already known, updated metadata". The orchestrator passes that boolean through to decide which jsonl to write to.
