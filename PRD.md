...
### US5: WriterAgent
- [ ] 모든 `tools/*.md`를 읽고 통합
- [ ] 비교 표 자동 생성 (모든 컬럼 채워짐, 빈 셀은 "unknown" 명시)
- [ ] 유스케이스별 매트릭스 (단일 GPU / 멀티 GPU / 엔터프라이즈 / 데이터 프라이버시 등)
- [ ] 이번 주 하이라이트 식별 (지난 주 폴더가 있으면 비교, 없으면 "첫 발행")
- [ ] 모든 사실 주장에 인용 (`[^N]` + sources.jsonl 매핑)
- [ ] 톤: 정보 제공 위주, 마케팅 언어 금지
